...
(function () {
  ...
  // --- build the cloud ---
  const COUNT = finePointer ? 64 : 38;          // fewer nodes on mobile
  ...
  function draw(ts) {
    ...
    for (let i = 0; i < COUNT; i++) {
      ...
      for (let j = i + 1; j < COUNT; j++) {
        ...
        if (d < LINK) {
          ...
          const alpha = (1 - d / LINK) * 0.34 * Math.max(0, (depth - 0.55) / 0.7);
    ...
    for (const p of order) {
      ...
      const twk = 0.55 + 0.45 * Math.sin(tw * p.n.sp + p.n.ph);
      const baseR = (p.n.big ? 2.6 : 1.7) * p.scale;
      const alpha = (p.n.big ? 0.95 : 0.7) * depth * twk;
      if (alpha <= 0.02) continue;
      if (p.n.big) {
        const g = ctx.createRadialGradient(p.sx, p.sy, 0, p.sx, p.sy, baseR * 4);
        g.addColorStop(0, 'rgba(157,185,255,' + (alpha * 0.5).toFixed(3) + ')');
        g.addColorStop(1, 'rgba(157,185,255,0)');
        ctx.fillStyle = g;
        ctx.beginPath(); ctx.arc(p.sx, p.sy, baseR * 4, 0, 6.2832); ctx.fill();
        ctx.fillStyle = 'rgba(190,208,255,' + alpha.toFixed(3) + ')';
      } else {
        ctx.fillStyle = 'rgba(186,202,255,' + alpha.toFixed(3) + ')';
      }
